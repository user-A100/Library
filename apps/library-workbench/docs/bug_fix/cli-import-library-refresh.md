...
**影响面**：应用打开本地 Vault 时，从外部 `library` CLI 导入论文。
...
- CLI 是外部进程，会写入 `papers/...` 和 `.library/catalog.sqlite`。
- Host watcher 旧逻辑过滤了整个 `.library/`，前端收不到 Catalog SQLite 变更。
- `useVaultFileEvents` 对外部结构变更只刷新文件树，不刷新 Library；因此新目录出现了，但 Catalog 元数据仍是旧缓存，标签只能回退到目录名 / ID。

## 修复

- watcher 放行 `.library/catalog.sqlite`、`catalog.sqlite-wal`、`catalog.sqlite-shm`、`catalog.sqlite-journal`，继续过滤其它 `.library/` 内部文件。
...
手动预期：应用打开 Vault 后运行 `library --json -v <vault> import id <arxiv-or-doi>`，新论文行应自动从目录 ID 更新为标题/作者，无需重开论文库。

## 后续清理

- 前端 `isCatalogStoragePath`（catalog.sqlite 事件分支）已在后续清理中删除：Host watcher 忽略所有 `.library/` 路径，该分支收不到事件，属死代码。
