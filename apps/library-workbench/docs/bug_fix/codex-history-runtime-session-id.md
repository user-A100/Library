...
**状态**：已修复
**Issue**：[#209](https://github.com/poco-ai/Library/issues/209)
...
错误中的 UUID 是 Library 为事件关联生成的 runtime session id，不是 Codex rollout/thread id。
...
2. 再次打开该历史项时，因为本地 `lines` 已为空，前端转而调用 `session/load`；旧实现传入 `item.id`（Library runtime id），而不是该项已经保存的 `providerSessionId`（Codex thread id）。Codex 自然找不到对应 rollout。
...
4. ACP 为每次 `agent_run_once` 请求生成新的 Library runtime id，即便请求通过 `session/resume` / `session/load` 续接同一个 provider session。前端原先按 runtime id 无条件新增历史项，未按 provider id 合并，因此一个 provider 对话被拆成多个列表项。
...
参考 [`zed-industries/zed`](https://github.com/zed-industries/zed) 的 ACP client 与回归测试，客户端必须在请求完成前准备好会话接收容器。Library Host 的 `ReplayBuilder` 已经能接住 response 前的更新并产出非空 `history.lines`，因此第二阶段空白发生在前端把完整快照写入共享 session store 的过程，不需要修改 ACP 协议层。
