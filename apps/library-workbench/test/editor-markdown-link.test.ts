...
describe("Markdown link input rule", () => {
	...
	it("converts long GitHub issue URLs character by character", () => {
		const editor = typeLink(
			"[issue](https://github.com/user-A100/Library/issues)",
		);
			...
			value: [
				{
					...
					children: [
						...
						{
							text: " then [issue](https://github.com/user-A100/Library/issues",
						},
					],
				},
			],
		});
		const tail = " then [issue](https://github.com/user-A100/Library/issues"
			.length;
		editor.tf.select({
	...
	it("ships LinkPlugin with insertText override converting long URLs", () => {
		...
		for (const c of "[issue](https://github.com/user-A100/Library/issues)") {
			editor.tf.insertText(c);
		}
		const flat = JSON.stringify(editor.children);
		expect(flat).toContain("github.com/user-A100/Library/issues");
