...
describe("isPairOfferUrl", () => {
	it("accepts a well-formed pair offer url", () => {
		expect(isPairOfferUrl("library://pair#offer=abc123")).toBe(true);
	});
	...
	it("rejects other hosts", () => {
		expect(isPairOfferUrl("library://open#offer=abc123")).toBe(false);
	});

	it("rejects missing offer fragment", () => {
		expect(isPairOfferUrl("library://pair#token=abc123")).toBe(false);
	});
