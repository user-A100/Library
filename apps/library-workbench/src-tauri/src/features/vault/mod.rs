...
3. If there is no TeX and no `PAPER.md`, run `library paper parse {paper}` to generate `PAPER.md` with LiteParse, then read the generated file.
...
- **Math in Markdown (Library / KaTeX):** write real math delimiters so NOTES render.
...
/// Bundled library-cli skill (headless vault discover/import via CLI), POSIX variant.
pub const LIBRARY_CLI_SKILL: &str =
    include_str!("../../../../templates/vault/.agents/skills/library-cli/SKILL.md");

/// Bundled library-cli skill (headless vault discover/import via CLI), Windows variant.
pub const LIBRARY_CLI_SKILL_WINDOWS: &str =
    include_str!("../../../../templates/vault/.agents/skills/library-cli/SKILL-windows.md");
...
pub const LIBRARY_CLI_SKILL_FOR_PLATFORM: &str = if cfg!(windows) {
    LIBRARY_CLI_SKILL_WINDOWS
} else {
    LIBRARY_CLI_SKILL
};

/// Bundled vault-normalizer skill (directory migration into Library layout).
...
pub(crate) fn bundled_skill_files() -> &'static [(&'static str, &'static str)] {
    &[
        ...
        (
            ".agents/skills/library-cli/SKILL.md",
            LIBRARY_CLI_SKILL_FOR_PLATFORM,
        ),
        (
...
/// Parse optional integer `version:` from YAML frontmatter (Library managed
...
///
/// Creates: `papers/`, `notes/`, `.library/`, `.agents/` (+ `skills/`),
...
/// initializes `.library/catalog.sqlite`.
...
pub fn ensure_vault(path: &Path, locale: &str) -> Result<CreateVaultResult, AppError> {
    ...
    for dir in ["papers", "notes", ".library", ".agents", ".agents/skills"] {
    ...
    if !db_existed && db_path.exists() {
        created.push(".library/catalog.sqlite".into());
    }
...
/// Create Library vault skeleton. Creates the root directory when missing;
...
mod tests {
    ...
    fn create_vault_scaffolds_dirs_and_catalog() {
        let dir = env::temp_dir().join(format!("library-vault-create-{}", std::process::id()));
        ...
        assert!(dir.join(".agents/skills/library-cli/SKILL.md").is_file());
        ...
        assert!(dir.join(".library/catalog.sqlite").is_file());
    ...
    #[test]
    fn library_cli_skill_matches_host_platform() {
        let (_, content) = bundled_skill_files()
            .iter()
            .find(|(rel, _)| *rel == ".agents/skills/library-cli/SKILL.md")
            .expect("library-cli skill bundled");
        #[cfg(windows)]
        {
            assert!(
                content.contains("library-cli.cmd"),
                "Windows variant expected"
            );
            assert!(
                content.contains("LIBRARY_VAULT"),
        ...
        {
            assert!(
                content.contains("~/.local/bin/library"),
    ...
    /// `LIBRARY_TEST_VAULT_PATH=$HOME/Downloads/library-from-rust cargo test create_vault_at_env_path -- --ignored --nocapture`
    #[test]
    #[ignore = "set LIBRARY_TEST_VAULT_PATH to write a real vault (e.g. under Downloads)"]
    fn create_vault_at_env_path() {
        let raw = env::var("LIBRARY_TEST_VAULT_PATH").expect("set LIBRARY_TEST_VAULT_PATH");
