...
> [!NOTE]
> **两套评测体系**：应用内 TRACE-Eval（八维）内嵌于产品，用于每次回答的实时自检；面向大赛的离线完整评测为 **D1–D7 七维**（见下表与 [`evaluation/`](evaluation/README.md)）。样本集 **31 用例 = 27 份模型回答样本 + 4 项 CLI 计时任务**；方案文档中的「120 任务」为早期规划量，实际交付以 31 用例为准。
...
- 本仓库自有源码以 **AGPL-3.0** 授权（见 [`LICENSE`](LICENSE)），与内置 AGPL 组件的授权义务一致；
